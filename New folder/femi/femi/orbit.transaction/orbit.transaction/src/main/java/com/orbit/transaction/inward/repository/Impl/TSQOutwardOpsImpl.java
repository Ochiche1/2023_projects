...
public class TSQOutwardOpsImpl implements TSQOutwardOps {
    ...
    public ResponseModel postTransactions(TransactionProperties properties) {
        ...
        if(errorCode.trim().equals("-50")){
            responseModel.setResponseCode(ResponseConstants.ACCOUNT_NUMBER_NOT_FOUND_CODE);
            responseModel.setResponseMessage(ResponseConstants.ACCOUNT_NUMBER_NOT_FOUND);
        }
        if(errorCode.trim().equals("-11701")){
            responseModel.setResponseCode(ResponseConstants.DORMANT_ACCOUNT_NUMBER_CODE);
            responseModel.setResponseMessage(ResponseConstants.DORMANT_ACCOUNT_NUMBER);
        }
    ...
    private Map<String, Object> executeStoredProcedure(TransactionProperties properties) {
        ...
        SqlParameterSource inputParams = new MapSqlParameterSource()
            ...
            .addValue("acctype", properties.getAccttype(), Types.VARCHAR)
            ...
            .addValue("trandate", properties.getTrandate(), Types.VARCHAR)
            .addValue("error_code", 0, Types.INTEGER)
            .addValue("error_message", "", Types.VARCHAR);
        return simpleJdbcCall.execute(inputParams);
    }
   // LocalDateTime.parse(properties.getTrandate(), formatter).toString())
    @Override
